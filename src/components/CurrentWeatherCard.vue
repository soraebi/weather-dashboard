      ...
      <div class="flex items-center justify-center space-x-8 my-8">
        <div class="text-center">
          <div class="text-6xl lg:text-7xl font-bold mb-2">
            {{ safeRound(currentWeather.temperature_2m || 0) }}°
          </div>
          <div class="text-xl lg:text-2xl opacity-90">
            {{ getWeatherInfo(currentWeather.weather_code || 0).description }}
          </div>
        </div>
        <div class="text-5xl lg:text-6xl">
          {{ getWeatherInfo(currentWeather.weather_code || 0).icon }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-3 lg:grid-cols-6 gap-4 mt-8">
        <div class="text-center">
          <div class="text-lg lg:text-xl font-semibold">
            {{ safeRound(currentWeather.relative_humidity_2m || 0) }}%
          </div>
          <div class="text-sm opacity-70">湿度</div>
        </div>
        <div class="text-center">
          <div class="text-lg lg:text-xl font-semibold">
            {{ safeRound(currentWeather.pressure_msl || 0) }} hPa
          </div>
          <div class="text-sm opacity-70">気圧</div>
        </div>
        <div class="text-center">
          <div class="text-lg lg:text-xl font-semibold">
            {{ safeRound((currentWeather.wind_speed_10m || 0) / 3.6, 1) }} m/s
          </div>
          <div class="text-sm opacity-70">風速</div>
        </div>
        <div class="text-center">
          <div class="text-lg lg:text-xl font-semibold">
            {{ getWindDirection(currentWeather.wind_direction_10m || 0) }}
          </div>
          <div class="text-sm opacity-70">風向</div>
        </div>
        <div class="text-center">
          <div class="text-lg lg:text-xl font-semibold">
            {{ safeRound(currentWeather.precipitation || 0, 1) }} mm
          </div>
